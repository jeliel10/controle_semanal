...
class Functions():
    ...
    def montaTabelas(self):
        ...
        self.cursor.execute("""
                        CREATE TABLE IF NOT EXISTS controle(
                            cliente VARCHAR(200) NOT NULL PRIMARY KEY,
                            sistema VARCHAR(100) NOT NULL,
                            data_registro DATE NOT NULL,
                            data_finalizado VARCHAR(10),
    ...
    def cadastrarTarefa(self):
        ...
        self.conectar_bd()
        self.cursor.execute("""
                        INSERT INTO controle (cliente, sistema, data_registro, data_finalizado, observacao)
                         VALUES (%s, %s, %s, %s, %s)""", (self.client, self.system, self.date_registro, self.date_finish, self.observation))
        self.conn.commit()
        self.desconecta_bd()
        self.select_bd()
        self.limpar_tela()
    ...
    def OnDoubleClick(self, event):
        ...
        for n in self.list.selection():
            ...
            self.entry_client.insert(END, col1)
            self.entry_system.insert(END, col2)
            self.entry_date_registro.insert(END, col3)
            self.entry_date_finish.insert(END, col4)
            self.entry_observation.insert(END, col5)
    ...
    def delete_client(self):
        ...
        self.conectar_bd()
        self.cursor.execute("""DELETE FROM controle WHERE cliente = %s """, (self.client, ))
        self.conn.commit()
        self.desconecta_bd()
        self.limpar_tela()
        self.select_bd()
    ...
    def update_client(self):
        ...
        self.conectar_bd()
        self.cursor.execute("""
                        UPDATE  controle SET sistema = %s, data_registro = %s, data_finalizado = %s, observacao = %s
                        WHERE cliente = %s""", (self.system, self.date_registro, self.date_finish, self.observation, self.client))
        self.conn.commit()
        self.desconecta_bd()
        self.select_bd()
        self.limpar_tela()
    ...
    def search_incomplete(self):
        ...
        # self.date_registro = self.entry_date_registro.get()
        #
        # self.month = self.date_registro[3:5]
        # self.year = self.date_registro[6:10]
        #
        # self.data_inicial = '{}/{}/{}'.format(self.year, self.month, 1)
        # self.data_inicial = '{}/{}/{}'.format(1, self.month, self.year)
        # self.dia_fev = 0
        ...
        data_incomplete = '{}/{}/{}'.format(1010,10,10)
        data_incomplete2 = '{}/{}/{}'.format(1010,10,10)
        ...
        self.cursor.execute("""
                                ...
                                WHERE data_finalizado BETWEEN %s AND %s
                                ORDER BY cliente ASC
                                """, (data_incomplete, data_incomplete2))
        lista = self.cursor.fetchall()
        lista_melhorada3 = []
        for i in lista:
            aux = []
            for k in range(0, len(i)):
                if k == 2 or k == 3:
                    ...
                    aux.append(i[k])
            lista_melhorada3.append(aux)

        for i in lista_melhorada3:
            self.list.insert("", END, values=i)
        self.limpar_tela()
        self.desconecta_bd()
...
class Controler(Functions):
    ...
    def create_labels(self):
        ...
        self.lb_client.place(rely= 0.08, relx= 0.01, relwidth= 0.065, relheight= 0.1)

        self.entry_client = Entry(self.frame_information)
        self.entry_client.place(rely= 0.2, relx= 0.01, relwidth= 0.13)
        ...
        self.lb_system.place(rely= 0.08, relx= 0.17, relwidth= 0.073, relheight= 0.1)

        self.entry_system = Entry(self.frame_information)
        self.entry_system.place(rely= 0.2, relx= 0.17, relwidth=0.08)
        ...
        self.lb_date_registro.place(rely= 0.4, relx= 0.01, relwidth= 0.122, relheight= 0.1)

        self.entry_date_registro = Entry(self.frame_information)
        self.entry_date_registro.place(rely= 0.52, relx= 0.01, relwidth= 0.122)
        self.entry_date_registro.bind("<KeyRelease>", self.format_date_registro)


        self.lb_date_finish = Label(self.frame_information, text= "Data Finalizado", font= "-weight bold -size 10", bg= self.color_frames, fg= self.color_names)
        self.lb_date_finish.place(rely= 0.4, relx= 0.17, relwidth= 0.14, relheight= 0.1)

        self.entry_date_finish = Entry(self.frame_information)
        self.entry_date_finish.place(rely= 0.52, relx= 0.17, relwidth= 0.122)
        self.entry_date_finish.bind("<KeyRelease>", self.format_date_finish)
    ...
    def frame_list_activity(self):
        self.list = ttk.Treeview(self.frame_activity, height= 3, columns= ("col1", "col2", "col3", "col4", "col5"))

        self.list.heading("#0", text= "")
        self.list.heading("#1", text= "Cliente")
        self.list.heading("#2", text= "Sistema")
        self.list.heading("#3", text= "Data Registro")
        self.list.heading("#4", text= "Data Finalizado")
        self.list.heading("#5", text= "Anotações")

        self.list.column("#0", width=1)
        self.list.column("#1", width= 50)
        self.list.column("#2", width= 30)
        self.list.column("#3", width= 47)
        self.list.column("#4", width= 53)
        self.list.column("#5", width= 310)
